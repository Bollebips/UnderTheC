...
bool RendererInit(Renderer *renderer) {
  ...
  int height = 720;
  GLFWwindow *window =
      glfwCreateWindow(width, height, "Voxel renderer", NULL, NULL);
  ...
  GLuint computeShaderHandle =
      CreateComputeShader("resources/voxelShader.glsl");
...
void Render(Renderer *renderer) {
  ...
  GLint cameraTransformAttribute =
      glGetUniformLocation(renderer->ShaderProgram, "cameraTransform");
  GLint octreeAttribute =
      glGetUniformLocation(renderer->ShaderProgram, "octree");
  ...
  while (!glfwWindowShouldClose(renderer->Window)) {
    ...
    averageDeltaTime = ((averageDeltaTime * deltaTimeSampleCount) + deltaTime) /
                       (deltaTimeSampleCount + 1);
    ...
    glUniformMatrix4fv(cameraTransformAttribute, 1, GL_FALSE,
                       renderer->Camera.Transform.raw[0]);
    glUniform1iv(octreeAttribute, 9, renderer->VoxelChunk.Octree.Items);

    GLuint numGroupsX =
        (renderer->Texture.Width + workGroupSizeX - 1) / workGroupSizeX;
    GLuint numGroupsY =
        (renderer->Texture.Height + workGroupSizeY - 1) / workGroupSizeY;

    glBindImageTexture(0, renderer->Texture.Handle, 0, GL_FALSE, 0,
...
void AttachTextureToFramebuffer(Renderer *renderer) {
  glNamedFramebufferTexture(renderer->Framebuffer, GL_COLOR_ATTACHMENT0,
                            renderer->Texture.Handle, 0);
  ...
  renderer->ViewportDimensionsAttribute =
      glGetUniformLocation(renderer->ShaderProgram, "viewportDimensions");
  float viewportHeight = renderer->Camera.NearPlaneDistance *
                         tanf(glm_rad(renderer->Camera.VerticalFov * 0.5f)) *
                         2.0f;
  float aspectRatio =
      ((float)renderer->Texture.Width / (float)renderer->Texture.Height);
  vec4 viewportDimensions = {viewportHeight * aspectRatio, viewportHeight,
                             renderer->Camera.NearPlaneDistance,
                             renderer->Camera.FarPlaneDistance};

  glUniform4fv(renderer->ViewportDimensionsAttribute, 1,
               (const GLfloat *)&viewportDimensions);
}

void BlitFramebufferToSwapchain(const GLuint framebuffer,
